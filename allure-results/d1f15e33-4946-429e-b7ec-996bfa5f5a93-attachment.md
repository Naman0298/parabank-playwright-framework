...
- Location: tests\API\responseTimeValidation.spec.ts:8:5

# Error details

```
Error: Failed to reach authenticated state
```
...
```ts
  ...
  2  | export async function ensureAuthenticated(page: Page) {
  3  | 
  4  |     const logoutLink = page.getByRole('link', { name: 'Log Out' });
  5  | 
  6  |     for (let i = 0; i < 3; i++) {
  7  | 
  8  |         if (await logoutLink.isVisible().catch(() => false)) { return; }
  9  | 
  10 |         await page.goto('https://parabank.parasoft.com/parabank/overview.htm');
  11 | 
  12 |         await page.waitForLoadState('networkidle');
  13 |     }
> 14 |     throw new Error('Failed to reach authenticated state');
     |           ^ Error: Failed to reach authenticated state
  15 | }
```